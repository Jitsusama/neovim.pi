...
describe("neovim-pi.file", function()
  ...
  describe("open()", function()
    ...
    it("opens in a vsplit distinct from the primary stage", function()
      local path = temp_file({ "a", "b" })
      local result = file.open(path, { mode = "vsplit" })
      assert.is_true(stage.owns(result.win))
      assert.are_not.equal(stage.current(), result.win)
      assert.are.equal(result.bufnr, vim.api.nvim_win_get_buf(result.win))
    end)

    it("positions the cursor when given line and col", function()
      local path = temp_file({ "first line", "second line" })
      local result = file.open(path, { line = 2, col = 3 })
      assert.are.same({ 2, 3 }, vim.api.nvim_win_get_cursor(result.win))
    end)
  end)
  ...
  describe("reload()", function()
    it("reloads a buffer pi owns, picking up changes made on disk", function()
      local path = temp_file({ "original" })
      local opened = file.open(path)
      vim.fn.writefile({ "changed on disk" }, path)

      local result = file.reload(opened.bufnr)

      assert.is_true(result.ok)
      assert.are.same({ "changed on disk" }, vim.api.nvim_buf_get_lines(opened.bufnr, 0, -1, false))
    end)

    it("refuses to reload a buffer pi does not own", function()
      local bufnr = vim.api.nvim_create_buf(false, true)
      local result = file.reload(bufnr)
      assert.is_false(result.ok)
    end)

    it("refuses a modified buffer until forced, then discards the edit", function()
      local path = temp_file({ "original" })
      local opened = file.open(path)
      text.set_range(opened.bufnr, 1, 0, 1, 8, "unsaved")
      vim.fn.writefile({ "disk content" }, path)
      assert.is_true(vim.bo[opened.bufnr].modified)

      local refused = file.reload(opened.bufnr)
      assert.is_false(refused.ok)
      assert.is_true(refused.modified)
      assert.is_true(vim.bo[opened.bufnr].modified)

      local forced = file.reload(opened.bufnr, true)
      assert.is_true(forced.ok)
      assert.are.same({ "disk content" }, vim.api.nvim_buf_get_lines(opened.bufnr, 0, -1, false))
    end)
  end)
end)
